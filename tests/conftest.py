...
import psycopg2
from app.v1.schema import user_schema, todo_schema
...
def postgresql_connection():
    ...
    con = psycopg2.connect(
        f"host='{settings.db_host}' port='{settings.db_port}' dbname='postgres' user='{settings.db_user}' password='{settings.db_pass}'")
...
def pytest_sessionstart(session):
    ...
    with db:
        db.create_tables([user_schema.User, todo_schema.Todo])
